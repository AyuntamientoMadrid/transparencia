...
describe ExcelImporters::Assets, clean_as_group: true do
  describe '#import', clean_as_group: true do
    before(:all) do
      logger = ExcelImporters::Base.new_default_logger
      ...
      ExcelImporters::Assets.new('./spec/fixtures/files/assets.xls', 'inicial', logger).import
      @declaration = Person.where(personal_code: 2379).first.assets_declarations.first
    end
